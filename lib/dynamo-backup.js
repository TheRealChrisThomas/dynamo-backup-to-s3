...
function DynamoBackup(options) {
    ...
    if (this.awsAccessKey && this.awsSecretKey && this.awsProfile) {
        console.log("Please specify either an access key and secret access key or a profile. Do not send both.");
        process.exit(-2);
    }
    ...
    if (this.awsProfile) {
        var creds = new AWS.SharediniFileCredentials({profile: params.awsProfile});
